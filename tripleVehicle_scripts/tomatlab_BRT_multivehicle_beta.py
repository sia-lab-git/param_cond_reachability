...
vehicle_betas_01 = [rad]
vehicle_betas_02 = [rad]
vehicle_betas_12 = [0]
...
fig_valfunc = plt.figure(figsize=(5, 5))
...
# Time coordinates
time_coords = torch.ones(mgrid_coords.shape[0], 1) * tt
...
coords_beta01 = vehicle_betas_01[0] * torch.ones(mgrid_coords.shape[0], 1)
coords_beta02 = vehicle_betas_02[0] * torch.ones(mgrid_coords.shape[0], 1)
coords_beta12 = vehicle_betas_12[0] * torch.ones(mgrid_coords.shape[0], 1)
...
ax_valfunc = fig_valfunc.add_subplot(1, 1, 1)
...
fig.suptitle('t=%.2f' % (tt), fontsize=26)
fig_valfunc.suptitle('t=%.2f' % (tt), fontsize=26)

fig.savefig(os.path.join(root_path, 'brt_t_%.2d_R_%.2d.png' % (100*tt,10*rad)))
fig_valfunc.savefig(os.path.join(root_path, 'val_t_%.2d_R_%.2d.png' % (100*tt,10*rad)))
scipy.io.savemat(os.path.join(root_path, 'brt_t_%.2d_R_%.2d.mat' % (100*tt,10*rad)), {'model_out': model_out})