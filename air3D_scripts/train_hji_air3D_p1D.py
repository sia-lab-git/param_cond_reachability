...
p.add_argument('--experiment_name', type=str, default='Air3Dp1D_reference', required=False,
               help='Name of subdirectory in logging_root where summaries and checkpoints will be saved.')

# General training options
p.add_argument('--batch_size', type=int, default=32)
p.add_argument('--lr', type=float, default=2e-5, help='learning rate. default=2e-5')
p.add_argument('--num_epochs', type=int, default=120000,
...
p.add_argument('--velocity', type=float, default=0.75, required=False, help='Speed of the dubins car')#
p.add_argument('--omega_max', type=float, default=3.0, required=False, help='Turn rate of the car')#
...
dataset = dataio.ReachabilityAir3Dp1DSource(numpoints=65000, collisionR=opt.collisionR, velocity=opt.velocity, 
...
# Define the loss
loss_fn = loss_functions.initialize_hji_air3Dp1D(dataset, opt.minWith)
...
def val_fn(model, ckpt_dir, epoch):
  ...
  for i in range(num_times):
    ...
    for j in range(num_thetas):
      ...
      omega_a_coords = torch.ones(mgrid_coords.shape[0], 1) * 3.0  #plot for wa=3
